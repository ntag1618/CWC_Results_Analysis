...
Area_change_plot <- function(.data) {
  .data %>%
    ...
    ggplot(., aes(x=loss_gain, y = Area_m2, fill = signs_YNf)) +
    geom_bar(width=0.8, stat="identity", position=position_dodge(width=0.9), colour='black', alpha=0.5, lwd=0.1)+
    geom_text(aes(label=paste(round(Perc_Area, 1), "%")),position=position_dodge(width=0.95), vjust=-1, size=2.5) +
    facet_wrap(~LoD_method) +
    labs(y=bquote('Area with Elevation Change ' ~(m^2)), x= 'Direction of Elevation Change') +
    scale_fill_manual("Foraging Observed", values=c('#AC4EC8', '#60C84E')) 
